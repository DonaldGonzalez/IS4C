...
class RotateDTransTask extends FannieTask {

	public $nice_name = 'Rotate Transaction Data';
	public $help_info = 'This script archives transaction data. The main
   reason for rotating transaction data into
   multiple snapshot tables is speed. A single large
   transaction table eventually becomes slow. The 
   rotation applied here is as follows:
   ...
   just the current day\'s data. ';
	...
	function run(){
		global $FANNIE_TRANS_DB, $FANNIE_ARCHIVE_REMOTE, $FANNIE_ARCHIVE_SERVER,
			$FANNIE_ARCHIVE_DBMS,$FANNIE_ARCHIVE_USER,
			$FANNIE_ARCHIVE_PW, $FANNIE_ARCHIVE_DB,
			$FANNIE_ARCHIVE_METHOD, $FANNIE_SERVER_DBMS;
		set_time_limit(0);
		...
		if ($chk1 === false)
			echo $this->cron_msg("Error loading data into transarchive");
		elseif ($chk2 === false)
			echo $this->cron_msg("Error trimming transarchive");
		else
			echo $this->cron_msg("Data rotated into transarchive");
		...
		if ($chk1 === false || $chk2 === false)
			echo $this->cron_msg("Error reloading dlog_15");
		else
			echo $this->cron_msg("Success reloading dlog_15");
		...
		*/
		if ($FANNIE_ARCHIVE_REMOTE){
			...
			if (!$sql->table_exists($table)){
				$this->createArchive($table,$sql);
				$this->createViews($dstr,$sql);
			}
			...
			if ($FANNIE_ARCHIVE_METHOD == "partitions" && $FANNIE_SERVER_DBMS == "MYSQL"){
				...
				if (date('j') == 1){
					...
					$newR = $sql->query($newQ);
					if ($newR === false)
						echo $this->cron_msg("Error creating new partition $p");
				}
				...
				*/
				...
				if ($chk1 === false || $chk2 === false)
					echo $this->cron_msg("Error creating new archive $table");
				else
					echo $this->cron_msg("Created new table $table and archived dtransactions");
				$this->createViews($dstr,$sql);
			}
			else {
				...
				if ($chk === false)
					echo $this->cron_msg("Error archiving dtransactions");
				else
					echo $this->cron_msg("Success archiving dtransactions");
			}
		...
		$sql = FannieDB::get($FANNIE_TRANS_DB);
		$chk = $sql->query("TRUNCATE TABLE dtransactions",$FANNIE_TRANS_DB);
		if ($chk === false)
			echo $this->cron_msg("Error truncating dtransactions");
		else
			echo $this->cron_msg("Success truncating dtransactions");
	}
	...
	function createViews($dstr,$db){
		...
		if ($chk === false)
			echo $this->cron_msg("Error creating dlog view for new archive table");
		...
		if ($chk === false)
			echo $this->cron_msg("Error creating receipt view for new archive table");
		...
		if ($chk === false)
			echo $this->cron_msg("Error creating receipt header view for new archive table");
	}
}

if (php_sapi_name() === 'cli'){
	$obj = new RotateDTransTask();	
	$obj->run();
}
