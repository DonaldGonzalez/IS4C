...
class PUDeptTask extends FannieTask {

	public $nice_name = 'Log Product Dept Updates';	
   	public $help_info = 'This script scans prodUpdate for instances
   where a product\'s department actually changed
   and logs those into prodDepartmentHistory.
   ...
   ends up having a ton of entries.';
	...
	function run(){
		...
		while($w = $sql->fetch_row($r)){
			...
			if ($prevDept != $w['dept']){ // dept changed
