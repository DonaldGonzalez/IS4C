...
class HqDTransTask extends FannieTask {

	public $nice_name = 'Send Transactions To HQ';
   	public $help_info = 'This script ships transaction data
   from local store dtransactions to the master
   store. Info is written directly to the master
   store\'s archive tables, not master.dtransactions.
   ...
   dtransactions)';
	...
	function run(){
		global $FANNIE_NUM_STORES, $FANNIE_MASTER_STORE,
			$FANNIE_STORES, $FANNIE_MASTER_ARCH_DB,
			$FANNIE_TRANS_DB;
		set_time_limit(0);
		...
		$sql->add_connection($minfo['host'],$minfo['type'],$FANNIE_MASTER_ARCH_DB,
				$minfo['user'],$minfo['pw']);
