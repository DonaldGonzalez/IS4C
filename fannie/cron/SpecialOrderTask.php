...
class SpecialOrderTask extends FannieTask {

	public $nice_name = 'Close Special Orders';
   	public $help_info = 'This script checks for special order items that have
   been picked up and moves them from PendingSpecialOrder
   to CompleteSpecialOrder';
	...
	function run(){
		...
		echo $this->cron_msg("Found ".count($order_ids)." order items");
		...
		$chkR = $sql->query($chkQ);
		echo $this->cron_msg("Missed on ".$sql->num_rows($chkR)." items");
		...
		echo $this->cron_msg("Found ".count($todo)." new order headers");
		...
		echo $this->cron_msg("Finishing $clean orders");
