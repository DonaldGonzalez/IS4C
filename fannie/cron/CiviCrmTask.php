...
824	Individual	0		Debbie		Adams					80 Ritchie Ave.			Toronto	M6R 2J9	1108	1039		debbieadams@sympatico.ca	0	0	835	4	0	2010-03-12	2010-03-12		100.00	7	1	2010-03-12 00:00:00		0
 ...
 03Feb13 Andy Wrap functionality in FannieTask class
...
// Connection id's, etc.
include_once(dirname(__FILE__).'/../config_wefc.php');
...
class CiviCrmTask extends FannieTask {
	public $nice_name = 'WEFC CRM Sync';
	public $help_info = '(EL has a better idea what goes here)';
...
// --functions } - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	function run(){
		global $CIVICRM_SERVER,$CIVICRM_SERVER_DBMS,$CIVICRM_DB,
		$CIVICRM_SERVER_USER,$CIVICRM_SERVER_PW,$FANNIE_OP_DB;
// --PREP - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
// What does this do?
// Bypasses PHP's execution time limit so scripts can run longer than ~30 seconds
set_time_limit(0);
...
//$row = $sql->fetch_row($res);
	...
	$this->dieHere("Little test c OK, bailing ...");
...
$dbConn2 = FannieDB::get($FANNIE_OP_DB);
...
if (FALSE) {
	$resultString = $this->clearIS4C();
	if ( $resultString != "OK" ) {
		$this->dieHere("$resultString");
	}
...
if ( $version = 1 ) {
	...
	if ( ! $outer ) {
		//$dbConn->close();
		$this->dieHere("Could not open $outFile\n");
	}
}

$logger = fopen("$logFile", "a");
if ( ! $logger ) {
	$this->dieHere("Could not open $logFile\n");
}
...
//echo "To DML is4cOp >${is4cOp}< CardNo >$custdata[CardNo]<\n";
			...
			if ( True || $is4cOp1 == "insert" ) {
				// Insert the records for this Individual, Household or Organization.
				$resultString = $this->insertToIS4C();
				if ( $resultString != "OK" ) {
					//$dbConn->close();
					//$dbConn2->close();
					$this->dieHere("$resultString");
		...
		*/
		...
		$customers = $this->searchIS4C($custdata[CardNo]);
		// Error is in [0][0]
		...
		if ( count($customers) == 0 ) {
			...
			$is4cOp = $this->searchIS4C2($custdata[CardNo]);
			if ( preg_match("/^Error/", $is4cOp[0][0]) ) {
				$this->dieHere("{$is4cOp[0][0]}");
			}
			$updateCount++;
		}
...
;";
		...
		*/
		...
		$meminfo[street] = $this->fixAddress($row[street_address]);
		...
		$meminfo[city] = $this->fixCity($row[city]);
		$meminfo[state] = $this->getProvince($row[state_province_id], 1);
		$meminfo[zip] =  $this->fixPostalCode($row[postal_code]);
		$meminfo[phone] = $this->fixPhone($row[phone]);
...
;";
	...
	if ( ! $writeOK ) {
		//$dbConn->close();
		$this->dieHere("Could not write to $outFile\n");
	}
	$outCount++;
...
echo $this->cron_msg("Success updating members from CiviCRM: $message");
...
/**
  Only execute the task if the script is run from the command line.
  If run by cron (or manually) the task will complete but the web
  front end can include_once() this file without running the task. It's
  set up this way so web front end can safely access the class
  definition and show information ($nice_name, $help_info, etc)
*/
